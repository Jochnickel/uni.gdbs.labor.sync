...
// reader() reads blocks of the input file to buffers[0/1]
void reader(long my_id) {
  int x=0;
  while (BUFFERSIZE==read(testfile, (void *) &(buffers[x]), BUFFERSIZE)) {
    x=1-x; // toggles x between 0 and 1
  }
}

// write() looks for correct values in buffers[0/1], provided by readers()
